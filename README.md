...
│   └── tsconfig.json
├── public/                   # Next.js Static Assets (images, icons)
...
Create a `.env` file in the root folder for Next.js:
```bash
# In /.env
...
```
This runs:
- `npm run build:backend` (TypeScript `tsc` compiler)
- `npm run build:frontend` (Next.js production bundles)
...
2. Next.js is automatically detected; keep default build and output settings.
